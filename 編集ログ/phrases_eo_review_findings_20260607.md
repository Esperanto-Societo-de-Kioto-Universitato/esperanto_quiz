...
Google Driveフォールバックは旧 `minibaro` 3キーをmanifestから外し、新3キーは未登録の警告として残す。
Driveへの新WAVアップロードとmanifest再生成はユーザー側で実施予定。
...
音声(spomenka)・モバイルデータは完全一致で整備済み。Driveフォールバックは新3キー未登録の警告を残し、
ユーザー側アップロード後にmanifest再生成予定。データは専門的に高品質。
